...
# دوال إدارة الحسابات
# ============================================================
def get_user_accounts_file(user_id):
    return os.path.join(ACCOUNTS_DIR, f"{user_id}.json")

def load_accounts_data(user_id):
    file_path = get_user_accounts_file(user_id)
    if os.path.exists(file_path):
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                data = json.load(f)
                data.setdefault("active_image_index", -1)
                data.setdefault("active_video_index", -1)
                data.setdefault("active_image_edit_index", -1)
                data.setdefault("active_ocr_index", -1)
...
class AccountsView(discord.ui.View):
    ...
    async def show_status(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title="📊 حالة الحسابات", color=discord.Color.blue())
    ...
    async def show_account_detail(self, interaction: discord.Interaction, idx):
        ...
        embed = discord.Embed(title=f"📱 تفاصيل الحساب {idx + 1}", color=discord.Color.green())
...
@bot.event
async def on_ready():
    print(f"✅ ZEUS bot جاهز (v{BOT_VERSION})")
    try:
        synced = await bot.tree.sync()
        print(f"تمت مزامنة {len(synced)} أمر.")
    except Exception as e:
        print(f"فشل مزامنة الأوامر: {e}")
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="ZEUS | /help"))
...
async def extract(interaction: discord.Interaction):
    ...
    filename = f"extracted_{user_id}_{int(time.time())}.txt"
...
async def help_command(interaction: discord.Interaction):
    ...
    embed.add_field(
        name="🔹 /extract",
        value="يبدأ عملية استخراج النصوص من الصور.\n"
              "الخطوات:\n"
              "1. اختر وضع المعالجة (دقة عالية أو سرعة عالية).\n"
              "2. أرسل الصور (حتى 5) مباشرة، أو ملف ZIP، أو رابط Google Drive.\n"
              "3. سيقوم البوت بمعالجة كل صورة على حدة وإرسال ملف TXT يحتوي على النصوص مرتبة.",
        inline=False,
    )
